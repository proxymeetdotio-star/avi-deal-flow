...
export const convertLeadToMandate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const insert = {
      ...
      use_of_funds_detail: lead.use_of_funds_detail,
      years_in_operation: lead.years_in_operation,
      existing_debt: lead.existing_debt,
      cashflow_strength: lead.cashflow_strength,
      readiness_score_100: lead.readiness_score_100,
      fundability_rating: lead.fundability_rating,
      asset_class: null,
      geography: null,
...
const MandatePatch = z.object({
  ...
  patch: z.object({
    ...
    funding_purpose: z.string().max(200).nullable().optional(),
    use_of_funds_detail: z.string().max(4000).nullable().optional(),
    years_in_operation: z.number().int().nullable().optional(),
    existing_debt: z.number().nullable().optional(),
    cashflow_strength: z.string().max(40).nullable().optional(),
